interface CreateUtilsMybricksParams {
  /** 开启IO日志打印 */
  useLog: boolean;
}

const createUtilsMybricks = (params: CreateUtilsMybricksParams) => {
  const { useLog } = params;
  return `const log = (...args) => {
...
export class Subject {
  _values = []
  _observers = new Set()
  ${useLog ? "_log = undefined" : ""}

  constructor(params = {}) {
    ${useLog ? "this._log = params.log" : ""}
  }
  ...
  next(value) {
    ${useLog ? "log(this._log, JSON.stringify(value))" : ""}
...
const createReactiveInputHandler = (params) => {
  const { input, value, rels${useLog ? ", title" : ""} } = params;
  if (value?.subscribe) {
    value.subscribe((value) => {
      input(value, new Proxy({}, {
        get(_, key) {
          return (value) => {
            (rels[key] || (rels[key] = new Subject(${useLog ? '{log: `${EXE_TITLE_MAP["output"]} ${title} | ${key}`}' : ""}))).next(value)
          }
        }
      }))
    })
  ...
  return new Proxy({},
    {
      get(_, key) {
        return rels[key] || (rels[key] = new Subject(${useLog ? '{log: `${EXE_TITLE_MAP["output"]} ${title} | ${key}`}' : ""}))
      }
    }
  )
}
...
export const createInputsHandle = (that, init = false) => {
  if (init) {
    ...
    const { controller, columnVisibilityController${useLog ? ", title" : ""} } = that
    const { _inputEvents${useLog ? ", _comInfo" : ""} } = controller
    ${useLog ? "_comInfo.title = title;" : ""}
...
export const createJSHandle = (fn, options) => {
  ...
  const outputs = new Proxy({}, {
    ...
    get(_, key) {
      return (value) => {
        (rels[key] || (rels[key] = new Subject(${useLog ? '{log: `${EXE_TITLE_MAP["output"]} ${props.title} | ${key}`}' : ""}))).next(value)
      }
    }
  })
  ...
  const exeOutputs = new Proxy(
    ...
    {
      get(_, key) {
        return rels[key] || (rels[key] = new Subject(${useLog ? '{log: `${EXE_TITLE_MAP["output"]} ${props.title} | ${key}`}' : ""}))
      },
    },
  )
    ...
    if (args.length) {
      ...
      if (isJsMultipleInputs) {
        ...
        args.forEach((value, index) => {
          if (value?.subscribe) {
            value.subscribe((value) => {
              ${useLog ? 'log(`${EXE_TITLE_MAP["input"]} ${props.title} | ${props.inputs[index]}`, JSON.stringify(value));' : ""}
        ...
        if (value?.subscribe) {
          value.subscribe((value) => {
            ${useLog ? 'log(`${EXE_TITLE_MAP["input"]} ${props.title} | ${props.inputs[0]}`, JSON.stringify(value));' : ""}
            createReactiveInputHandler({
              input: controller,
              value,
              rels,
...
export default createUtilsMybricks;
