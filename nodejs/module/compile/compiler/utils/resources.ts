...
export class ResourceCollector {
  ...
  async outputToResources(appFolder: string): Promise<void> {
    ...
    const fetchUrl = async (url: string): Promise<Buffer> => {
      if (!urlCache.has(url)) {
        const file = await axios.get(url, { responseType: 'arraybuffer', timeout: 10 * 1000 }).then(response => Buffer.from(response.data))
    ...
    for (const [original, resource] of this.resources) {
      ...
      try {
        if (resource.type === 'url') {
          data = await fetchUrl(resource.value);
        } else {
          const base64Data = resource.value.split(',')[1];
          data = Buffer.from(base64Data, 'base64');
        }
      } catch (error) {
        const base64Data = FALLBACK_IMAGE.split(',')[1];
        data = Buffer.from(base64Data, 'base64');
      }
